...
ampl_prob_dir = "../ampl/"
ampl_probs = [Symbol(split(p, ".")[1]) for p in filter(x -> contains(x, ".nl") 
                                                       && x != "hs068.nl"
                                                       && x != "hs069.nl"
                                                       && x != "drcavty1.nl"
                                                       && x != "drcavty2.nl"
...
#s1, P1 = compare_solvers_Ampl(solvers1, ampl_prob_dir, ampl_probs, n_min, n_max, title = "First order: #f + #g ")
#save("s1P1.jld","s1",s1)

solvers2 = [ARCMA97_abs, ARCSpectral_abs, Ipopt_NMPB]
n_max = 500
s2, P2 = compare_solvers_Ampl(solvers2, ampl_prob_dir, ampl_probs, n_min, n_max, title = "Second order but only #f + #g ")
save("s2P2.jld","s2",s2)

#solvers3 = [LD_TNEWTON, ARCqKsparse, ST_TRsparse, TRKsparse] # trunk, # Hprod bugs, needs matrix
#n_max = 10000
#s3, P3 = compare_solvers_Ampl(solvers3, ampl_prob_dir, ampl_probs, n_min, n_max, title = "First order: #f + #g ")
#save("s3P3.jld","s3",s3)
